...
export default function App() {
  ...
  // Navigation
  const [activeTab, setActiveTab] = useState<'config' | 'estoque' | 'assinaturas'>('estoque');
  ...
  const playSubscriptionSound = () => {
    ...
    try {
      const audioCtx = new (window.AudioContext || (window as any).webkitAudioContext)();
      const playTone = (frequency: number, startOffset: number, duration: number, type: OscillatorType, volume: number) => {
        const osc = audioCtx.createOscillator();
        const gain = audioCtx.createGain();
      ...
      playTone(196, 0.00, 0.12, 'sawtooth', 0.14);
      playTone(880, 0.16, 0.10, 'square', 0.12);
      playTone(196, 0.30, 0.12, 'sawtooth', 0.14);
      playTone(987.77, 0.46, 0.24, 'square', 0.12);
    } catch (e) { }
  };
  ...
  if (!isUnlocked) {
    return <AuthLock onUnlock={() => setIsUnlocked(true)} />;
  }

  const handleDisableTutorial = () => {
    handleSaveSettings({
      ...settings,
      general: { ...settings.general, showTutorial: false }
    });
    setShowTutorialManual(false);
  };

  const handleSaveOnboard = () => {
    if (onboardName.trim()) {
      handleSaveSettings({
        ...settings,
        general: { ...settings.general, storeName: onboardName.trim() }
      });
    }
  ...
  return (
    ...
    >
      ...
      {/* TUTORIAL MODAL */}
      {(showTutorialManual || settings.general.showTutorial) && (
        <TutorialModal
          onClose={() => {
            setShowTutorialManual(false);
            if (settings.general.showTutorial) {
              handleSaveSettings({ ...settings, general: { ...settings.general, showTutorial: false } });
            }
          }}
      ...
      {showOnboard && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
          ...
          <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} className="relative w-full max-w-sm bg-white dark:bg-slate-900 rounded-3xl shadow-2xl p-6 z-[121]">
            <h2 className="text-xl font-black text-slate-800 dark:text-slate-100 mb-2">Bem-vindo(a)!</h2>
            <p className="text-sm text-slate-500 mb-5">Para personalizar o aplicativo, insira o nome da sua Loja ou Empreendimento.</p>
            <input type="text" placeholder="Sua Loja" value={onboardName} onChange={e => setOnboardName(e.target.value)} className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 mb-4" />
            <button onClick={handleSaveOnboard} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 rounded-xl transition-all cursor-pointer">Começar</button>
          </motion.div>
        </div>
      )}
      ...
      <button
        ...
        className="fixed bottom-6 right-6 z-[90] w-12 h-12 bg-indigo-600 hover:bg-indigo-700 text-white rounded-full flex items-center justify-center shadow-xl shadow-indigo-500/30 hover:scale-105 active:scale-95 transition-all cursor-pointer"
        title="Ver Tutorial"
      >
      ...
      {systemStatus.updater && systemStatus.updater.status !== 'none' && (() => {
        // Evaluate deferred updates
        if (systemStatus.updater.status === 'available') {
          const deferredAt = localStorage.getItem('update_deferred_at');
          if (deferredAt) {
            const timeSince = Date.now() - parseInt(deferredAt);
            if (timeSince < 24 * 60 * 60 * 1000) return null;
          }
        }

        const handleDeferUpdate = () => {
          localStorage.setItem('update_deferred_at', Date.now().toString());
          setSystemStatus(prev => ({ ...prev, updater: { status: 'none', progress: 0 } }));
        };
        ...
        return (
          <div className="absolute top-4 left-1/2 transform -translate-x-1/2 z-[100] animate-in fade-in slide-in-from-top-4 duration-300 w-[95%] max-w-lg">
            <div className="bg-slate-900/95 backdrop-blur-lg border border-slate-700/50 shadow-2xl rounded-2xl p-4 flex items-center space-x-4">
              {systemStatus.updater.status === 'available' ? (
                <>
                  <div className="bg-indigo-500/10 p-2 rounded-full text-indigo-400 flex-none border border-indigo-500/20">
                    <Download className="w-6 h-6" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-indigo-400 font-bold text-sm tracking-tight truncate">Atualização Encontrada</p>
                    <p className="text-xs text-slate-400 mt-0.5 leading-tight">Uma nova versão está disponível. DESEJA ATUALIZAR AGORA?</p>
                  </div>
                  <div className="flex gap-2 ml-2">
                    <button onClick={handleDeferUpdate} className="bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700 text-[10px] uppercase tracking-wider font-bold px-3 py-2 rounded-xl transition-all cursor-pointer">
                      NÃO
                    </button>
                    <button onClick={() => fetch('/api/system/updater-action', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'download' }) })} className="bg-indigo-500 hover:bg-indigo-600 text-white text-[10px] uppercase tracking-wider font-bold px-3 py-2 rounded-xl transition-all shadow-lg shadow-indigo-500/20 active:scale-95 whitespace-nowrap cursor-pointer">
                      SIM
                    </button>
                  </div>
                </>
                ...
                <>
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-emerald-400 font-bold text-sm tracking-tight truncate">Aplicando Atualização</p>
                    <p className="text-xs text-slate-400 mt-0.5 leading-tight">Preparando para fechar e abrir automaticamente...</p>
                  </div>
                  <div className="flex items-center space-x-3 text-slate-300 ml-2">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h1 className="text-base font-bold text-slate-900 dark:text-slate-100 tracking-tight flex items-center gap-1.5 leading-none">
              {settings.general.storeName || 'Global Stock'}
        ...
        {activeSubscriptionAlerts.length > 0 && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/80 backdrop-blur-sm p-4" >
            {activeSubscriptionAlerts.map(alert => (
              <motion.div key={`${alert.id}-${alert.stage}`} initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} className="bg-white dark:bg-slate-900 border-2 border-indigo-500 rounded-2xl p-6 shadow-2xl max-w-md w-full relative overflow-hidden" >
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h2 className="text-lg font-black text-slate-900 dark:text-white uppercase tracking-tight">Aviso de Assinatura</h2>
                    <p className="text-sm font-medium text-indigo-600 dark:text-indigo-300 mt-1 leading-tight">
                      {alert.platformName} - {alert.stageTitle}
                    </p>
                  </div>
                </div>
                <div className="mt-5 p-4 bg-slate-50 dark:bg-slate-950/50 border border-slate-200 dark:border-slate-800 rounded-xl space-y-2">
                  <div className="flex justify-between items-center border-b border-slate-200 dark:border-slate-800 pb-2 gap-4">
                    <span className="text-xs font-bold text-slate-500">Cliente</span>
                    <span className="text-xs font-black text-slate-800 dark:text-slate-200 text-right break-words">{alert.customerName}</span>
                  </div>
                  <div className="flex justify-between items-center border-b border-slate-200 dark:border-slate-800 pb-2 gap-4">
                    <span className="text-xs font-bold text-slate-500">Produto</span>
                    <span className="text-xs font-black text-slate-800 dark:text-slate-200 text-right break-words">{alert.productName}</span>
                  </div>
                  <div className="flex justify-between items-center pt-1 gap-4">
                    <span className="text-xs font-bold text-slate-500">Expira em</span>
                    <span className="text-[10px] font-black uppercase text-amber-600 dark:text-amber-400 bg-amber-500/10 px-2 py-0.5 rounded-full ring-1 ring-amber-500/20">
                      {alert.expiresAt ? new Date(alert.expiresAt).toLocaleString('pt-BR') : 'Sem data'}
                    </span>
                  </div>
                </div>
                <div className="mt-6 flex flex-col gap-2">
                  {alert.chatLink && (
                    <a href={alert.chatLink} target="_blank" rel="noreferrer" className="w-full py-2.5 text-center bg-slate-900 hover:bg-slate-800 dark:bg-slate-800 dark:hover:bg-slate-700 text-white font-bold text-xs uppercase tracking-wider rounded-xl transition-colors cursor-pointer" >
                      Abrir Chat do Cliente
                    </a>
                  )}
                  <button onClick={() => setActiveSubscriptionAlerts(prev => prev.filter(a => !(a.id === alert.id && a.stage === alert.stage)))} className="w-full py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs uppercase tracking-wider rounded-xl transition-colors shadow-lg shadow-indigo-600/20 cursor-pointer" >
                    Estou Ciente - Fechar Alerta
                  </button>
                </div>
              </motion.div>
