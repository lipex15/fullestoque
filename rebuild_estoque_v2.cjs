...
import {
  Database, Plus, Search, Trash2, Edit2, AlertTriangle, Key, ShoppingBag, Eye, Copy, Check, User, Clock, ChevronDown, ChevronUp, RefreshCw, X, Layers
} from 'lucide-react';
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 shadow-2xs space-y-4">
        ...
        {filteredProducts.length === 0 ? (
          <div className="py-12 border-2 border-dashed border-slate-200 dark:border-slate-800/80 rounded-xl text-center">
            ...
            <p className="text-[11px] text-slate-500 max-w-xs mx-auto mt-0.5">Cadastre um produto com itens de estoque para ativar o fluxo de entrega automática.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredProducts.map((p) => {
              ...
              return (
                <div
                  key={p.id}
                  className={\`border rounded-xl p-4 transition-all flex flex-col justify-between \${isSelected
                    ? 'bg-indigo-50/40 dark:bg-indigo-950/20 border-indigo-500 dark:border-indigo-800 shadow-xs'
                    : 'bg-white dark:bg-slate-900/60 border-slate-200 dark:border-slate-800/80 hover:border-slate-350 dark:hover:border-slate-700'
                    }\`}
                >
                  <div>
                    {/* Header: Platform Badge + Category */}
                    <div className="flex items-center justify-between gap-2">
                      <span className={\`text-[9px] font-bold uppercase px-2 py-0.5 rounded border \${p.platform === 'ggmax' ? 'bg-sky-50 text-sky-700 border-sky-200 dark:bg-sky-950/40 dark:text-sky-400 dark:border-sky-900/40' :
                    ...
                    <div className="flex items-center justify-between mt-4 bg-slate-50 dark:bg-slate-950/40 p-2.5 rounded-xl border border-slate-100 dark:border-slate-800/60">
                      <span className="text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wider">Estoque Disponível</span>
                      <div className="flex items-center gap-1.5">
                        <span className={\`text-sm font-black \${p.availableCount === 0 ? 'text-rose-600 dark:text-rose-400' :
                          isLow ? 'text-amber-500' : 'text-slate-900 dark:text-white'
                          }\`}>
                          {p.availableCount}
                        </span>
                        <span className="text-xs text-slate-400 font-bold">/</span>
                        <span className="text-xs font-bold text-slate-500 dark:text-slate-400">{p.totalCount} unids</span>
                      </div>
                    </div>

                    {/* Warning Banners */}
                    {isLow && (
                      <div className={\`mt-3 py-1 px-2.5 rounded-lg text-[9px] font-bold flex items-center gap-1 border \${p.availableCount === 0
                        ? 'bg-rose-50 border-rose-150 text-rose-700 dark:bg-rose-950/20 dark:border-rose-900/50 dark:text-rose-400'
                        : 'bg-amber-50 border-amber-150 text-amber-700 dark:bg-amber-950/20 dark:border-amber-900/50 dark:text-amber-400'
                        }\`}>
                        <AlertTriangle className="w-3.5 h-3.5 flex-shrink-0" />
                        <span>{p.availableCount === 0 ? "ESTOQUE ESGOTADO" : "AVISO: ESTOQUE BAIXO!"}</span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800/80 flex items-center justify-between">
                    <div className="flex items-center gap-1.5">
                      <button
                        ...
                        className="p-1.5 text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-lg cursor-pointer transition-all"
                      ...
                      <button
                        ...
                        className="p-1.5 text-slate-400 hover:text-rose-600 dark:hover:text-rose-400 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-lg cursor-pointer transition-all"
                    ...
                    <button
                      ...
                      className={\`px-2.5 py-1 text-[10px] font-extrabold rounded-lg flex items-center gap-1 cursor-pointer transition-all \${isSelected
                        ? 'bg-indigo-600 text-white'
                        : 'bg-slate-100 hover:bg-slate-200 text-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700'
                        }\`}
                    >
                      <span>{isSelected ? 'Fechar Itens' : 'Gerenciar Itens'}</span>
